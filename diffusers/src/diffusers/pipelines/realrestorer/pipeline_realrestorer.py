...
class RealRestorerPipeline(DiffusionPipeline):
    ...
    def _get_qwenvl_embeds(
        ...
        offload_dev = getattr(self, "_offload_device", None)
        if isinstance(offload_dev, str):
            offload_dev = torch.device(offload_dev)

        # Detect whether accelerate hooks are managing this module.
        has_hooks = hasattr(self.text_encoder, "_hf_hook")

        # Sequential offload keeps weights on ``meta``; inputs must go to the accelerator, but the module must not be
        # moved with ``.to()`` (use hooks / forward only). ``.to()`` from meta raises NotImplementedError.
        if model_device.type == "meta":
            text_encoder_device = self._accelerator_device_for_offloaded_io()
        elif has_hooks and offload_dev is not None:
            # model_cpu_offload: accelerate hooks will move the module to GPU on forward().
            # Inputs must be on the offload (GPU) device so they match after the hook fires.
            text_encoder_device = offload_dev
        else:
            text_encoder_device = offload_dev if offload_dev is not None else model_device

        # Only manually move if hooks are NOT active (manual placement or no offload).
        if not has_hooks and text_encoder_device != model_device and model_device.type != "meta":
            self.text_encoder.to(device=text_encoder_device, dtype=model_dtype)

        print(
            f"[RealRestorer] _get_qwenvl_embeds: model_device={model_device}, "
            f"text_encoder_device={text_encoder_device}, has_hooks={has_hooks}, offload_dev={offload_dev}",
            flush=True,
        )
        ...
        for idx, (prompt, ref_image, edit_type) in enumerate(zip(prompts, ref_images, edit_types)):
            ...
            del outputs  # Free all 28 layers of hidden states (~128MB GPU) immediately.
        ...
        # Only manually move back to CPU if hooks are NOT managing placement.
        if not has_hooks and offload_dev is not None and model_device.type != "meta":
    ...
    def _accelerator_device_for_offloaded_io(self) -> torch.device:
        """
        Under ``enable_sequential_cpu_offload``, component weights may live on ``meta``; activations must still be
        placed on the accelerator GPU so hooks can materialize weights and run the forward pass.
        """
        od = getattr(self, "_offload_device", None)
        if od is not None:
            return torch.device(od) if isinstance(od, str) else od
        ed = getattr(self, "_execution_device", None)
        if ed is not None:
            return torch.device(ed) if isinstance(ed, str) else ed
        if torch.cuda.is_available():
            return torch.device("cuda", torch.cuda.current_device())
        return torch.device("cpu")

    def _vae_activation_device(self) -> torch.device:
        """
        Device for VAE inputs/outputs.

        - ``enable_sequential_cpu_offload``: weights are on ``meta`` — place activations on the accelerator GPU
          (see ``_accelerator_device_for_offloaded_io``).
        - ``enable_model_cpu_offload``: weights are on ``cpu`` but the accelerate hook moves the *entire module*
          to GPU right before ``.forward()``.  Inputs must already be on GPU so they match the module after
          the hook fires.  ``_offload_device`` records which GPU that is.
        - ``full_gpu``: ``self.vae.device`` is already CUDA.
        """
        vae_dev = self.vae.device
        if vae_dev.type == "meta":
            return self._accelerator_device_for_offloaded_io()
        # model_cpu_offload: weights sit on cpu, but the hook will move them to _offload_device (GPU)
        # before forward().  Inputs must be on that same GPU device.
        od = getattr(self, "_offload_device", None)
        if od is not None and vae_dev.type == "cpu":
            return torch.device(od) if isinstance(od, str) else od
        return vae_dev
    ...
    def _move_vae_to_device(self, target_device: torch.device) -> bool:
        """
        Manually move the VAE to ``target_device`` when accelerate hooks are present.

        ``model_cpu_offload`` attaches hooks that fire on ``__call__()`` / ``forward()``,
        but RealRestorer's VAE uses ``.encode()`` / ``.decode()`` directly — the hooks
        never fire.  We must move the weights ourselves.

        Also offloads other components (text_encoder, transformer) to CPU first so we
        don't exceed VRAM when the VAE lands on the GPU.

        Returns ``True`` if we moved the VAE (caller should move it back after the op).
        """
        vae_dev = self.vae.device
        has_hook = hasattr(self.vae, "_hf_hook")

        if has_hook and vae_dev.type != target_device.type and vae_dev.type != "meta":
            # Offload other components to CPU first to free VRAM for the VAE.
            for name in ("text_encoder", "transformer"):
                comp = getattr(self, name, None)
                if comp is not None and hasattr(comp, "to"):
                    comp_dev = next(comp.parameters(), None)
                    if comp_dev is not None and comp_dev.device.type not in ("cpu", "meta"):
                        print(
                            f"[RealRestorer] VAE: offloading {name} to CPU first "
                            f"(was on {comp_dev.device})",
                            flush=True,
                        )
                        comp.to("cpu")
            ...
            print(
                f"[RealRestorer] VAE: manually moving {vae_dev} → {target_device} "
                f"(hook present but .encode()/.decode() bypass it)",
                flush=True,
            )
    ...
    def _encode_vae_image(self, image: torch.Tensor) -> torch.Tensor:
        ...
        act_dev = self._vae_activation_device()
        image = image.to(device=act_dev, dtype=vae_dtype)
        print(
            f"[RealRestorer] VAE encode forward (shape={tuple(image.shape)}, activation_device={act_dev})…",
            flush=True,
        )

        # model_cpu_offload: .encode() bypasses the accelerate hook → move VAE to GPU manually.
        moved = self._move_vae_to_device(act_dev)

        autocast_context = (
            torch.autocast(device_type=act_dev.type, enabled=False)
            if act_dev.type in {"cuda", "cpu"}
            else contextlib.nullcontext()
        )
        with autocast_context:
            encoded = self.vae.encode(image * 2 - 1)
        print("[RealRestorer] VAE encode forward finished.", flush=True)

        # Move VAE back to CPU to free VRAM for the next component (text_encoder / transformer).
        if moved:
            self.vae.to("cpu")
            torch.cuda.empty_cache()
    ...
    def _decode_vae_latents(self, latents: torch.Tensor) -> torch.Tensor:
        vae_dtype = self.vae.dtype
        act_dev = self._vae_activation_device()
        latents = latents.to(device=act_dev, dtype=vae_dtype)

        # model_cpu_offload: .decode() bypasses the accelerate hook → move VAE to GPU manually.
        moved = self._move_vae_to_device(act_dev)
    ...
    def _log_vram(label: str) -> None:
        """Print current CUDA VRAM usage for debugging."""
        if torch.cuda.is_available():
    ...
    def __call__(
        ...
        if image is None:
            ...
            print(
                f"[RealRestorer] VAE encoding reference image… (H×W={height}×{width}, size_level={size_level}; "
                "first run can take minutes: CUDA warmup + offload loading weights).",
                flush=True,
            )
            self._log_vram("before VAE encode")
            ...
            del ref_latents_tensor
            self._log_vram("after VAE encode (VAE back on CPU)")
            print(
                "[RealRestorer] Reference VAE encode done; next: Qwen2.5-VL prompt+image encoding (often minutes, no bar).",
                flush=True,
            )
        ...
        print(
            "[RealRestorer] Qwen2.5-VL encoding started (positives/negatives + optional image tokens). "
            "Silence here is normal — CPU offload may add long pauses.",
            flush=True,
        )
        self._log_vram("before text encoding")
        ...
        # ref_images_raw is no longer needed — free GPU memory before denoise.
        if ref_images_raw is not None:
            del ref_images_raw

        # CRITICAL: text_encoder (~15GB) is still on GPU after the hook moved it there.
        # The accelerate hook chain loads the NEXT module BEFORE offloading the previous one,
        # so both text_encoder (15GB) + transformer (14GB) would coexist on GPU briefly = 29GB > 24GB.
        # We must manually offload text_encoder NOW, before the denoise loop loads the transformer.
        te_dev = next(self.text_encoder.parameters()).device
        if te_dev.type not in ("cpu", "meta"):
            print(
                f"[RealRestorer] Manually offloading text_encoder ({te_dev}) → CPU to free VRAM for transformer",
                flush=True,
            )
            self.text_encoder.to("cpu")
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        self._log_vram("after text encoding + text_encoder offloaded")
        print("[RealRestorer] Qwen encoding finished; scheduler denoise will show tqdm.", flush=True)
        txt_ids = torch.zeros(
        ...
        # Free denoise intermediates before VAE decode.
        del prompt_embeds, prompt_mask, combined_img_ids, txt_ids
        if ref_latents is not None:
            del ref_latents

        # Offload transformer to CPU before VAE decode (same hook ordering issue).
        tr_dev = next(self.transformer.parameters()).device
        if tr_dev.type not in ("cpu", "meta"):
            print(
                f"[RealRestorer] Manually offloading transformer ({tr_dev}) → CPU to free VRAM for VAE decode",
                flush=True,
            )
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        self._log_vram("after denoise, before VAE decode")
        ...
        # Diffusers: call once at end of ``__call__`` for ``enable_model_cpu_offload`` (not from external wrappers).
